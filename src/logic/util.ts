...
export function getNumberInterpolator(
  animationInterpolation: AnimationInterpolation
): Interpolator<number> {
  switch (animationInterpolation) {
    case "cosine":
      return interpolateNumbersCosine;
    case "linear":
      return interpolateNumbersLinear;
    case "none":
      return undefined;
  }
}

export function getAngleInterpolator(
  animationInterpolation: AnimationInterpolation
): Interpolator<number> {
  switch (animationInterpolation) {
    case "cosine":
      return interpolateAngleLinear;
    case "linear":
      return interpolateAngleLinear;
    case "none":
      return undefined;
...
export function getVector2Interpolator(
  animationInterpolation: AnimationInterpolation
): Interpolator<Vector2> {
  switch (animationInterpolation) {
    case "cosine":
      return interpolateVectorsCosine;
    case "linear":
      return interpolateVectorsLinear;
    case "none":
      return undefined;
  }
}
...
function interpolateNumbersLinear(
  start: number,
  end: number,
  progress: number
) {
...
function interpolateNumbersCosine(
  start: number,
  end: number,
  percentage: number
) {
...
export function interpolateVectorsLinear(
  start: Vector2,
  end: Vector2,
  percentage: number
) {
...
export function interpolateAngleLinear(
  start: number,
  end: number,
  percentage: number
) {
...
export function interpolateVectorsCosine(
  start: Vector2,
  end: Vector2,
  percentage: number
) {
...
function correctEndAngle(start: number, end: number) {
  ...
  const bestEnd =
    minDif === dif0 ? end : minDif === dif1 ? alternativeEnd1 : alternativeEnd2;
...
export function onCanvas(
  ...
  return (
    lowerX >= 0 && upperX < screenWidth && lowerY >= 0 && upperY < screenHeight
  );
}
