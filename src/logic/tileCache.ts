...
class TileCache {
  ...
  private onTileDownloadingCountChangeEvent =
    new UnloadingEvent<OnTileDownloadingCountChangeListener>(
      "tileCacheDownloadingCountChange"
    );
  ...
  public static get instance(): TileCache {
    if (!this.isSupported) {
      throw new Error(
        "Using TileCache directly in this window is not supported. Use getTileCache instead."
      );
    }
  ...
  public getTileBitmap = (
    ...
    if (hit) {
      ...
      if (this.failedBitmapCache.has(key)) {
        ...
        const tileBitmapPromise = this.getTileBitmapFromServer(
          tileLevel,
          position
        );
        ...
        tileBitmapPromise.then((bitmap) => {
          ...
          this.onTileDownloadingCountChangeEvent.fire(
            this.downloadingBitmapCache.size
          );
        });
  ...
  public registerOnTileDownloadingCountChange =
    this.onTileDownloadingCountChangeEvent.register;
  ...
  private getTileBitmapFromServer = async (
    tileLevel: number,
    position: Vector2
  ) => {
