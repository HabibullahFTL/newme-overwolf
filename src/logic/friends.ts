    ...
    if (validatePlayerResponse(response as PlayerResponse)) {
      ...
      function addFriend(player: PlainTextPlayerData, channel: StoredChannel) {
        ...
        if (!friend) {
          friend = {
            colors: [],
            location: { x: player.l.x, y: player.l.y },
            name: player.n,
          };
...
export async function updateChannel(
  key: StoredChannel["id"],
  changes: Partial<StoredChannel>
) {
  const db = new ChannelsDatabase();
...
function validatePlayerResponse(
  response: PlayerResponse
): response is PlayerResponse {
  return (
    !!response &&
...
function validateChannelData(
  channelData: PlayerChannelResponseData
): channelData is PlayerChannelResponseData {
  return (
    !!channelData &&
...
function validatePlayerDataPlain(
  data: PlainTextPlayerData
): data is PlainTextPlayerData {
  return (
    typeof data === "object" &&
