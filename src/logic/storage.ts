...

{
  ...
  const deprecatedInterpolation = loadUntyped(
    deprecatedInterpolationKey,
    ""
  ) as DeprecatedInterpolation;
...
export const knownStorageScopes: KnownStorageScope[] = [
  "desktop",
  "icon",
  "inGame",
];
...
export function store<TKey extends keyof SimpleStorageSetting>(
  key: TKey,
  value: SimpleStorageSetting[TKey]
) {
...
export function load<TKey extends keyof SimpleStorageSetting>(
  key: TKey
): SimpleStorageSetting[TKey] {
  let storageKey: string = key;
...
function getIconPropertySettingKey(
  category: string,
...
export function storeIconConfiguration(
  category: string,
  type: string | undefined,
  property: IconProperty,
  value: boolean
...
export function loadIconConfiguration(
  category: string,
  type: string | undefined,
  property: IconProperty
) {
  ...
  return loadUntyped(
    key,
    getDefaultIconConfigurationValue(category, type, property)
  ) as boolean;
}

function getDefaultIconConfigurationValue(
  category: string,
  type: string | undefined,
  property: IconProperty
): boolean {
...
export function getStorageKeyScope(
  key: string
): [KnownStorageScope | undefined, string] {
...
export function deconstructIconStorageKey(identifier: string) {
  ...
  if (!iconProperties.includes(propertySplit[1] as IconProperty)) {
    return undefined;
  }
  const categoryAndType = propertySplit[0].split("--");
  if (categoryAndType.length < 1 || categoryAndType.length > 2) {
    return undefined;
  }
  return {
