...
const onPlayerDataUpdateEvent = new UnloadingEvent<OnPlayerDataUpdateListener>(
  "onPlayerDataUpdate"
);
...
export const registerEventCallback = isBackground
  ...
  : (overwolf.windows.getMainWindow() as OverwolfHookWindow)
      .NWMM_registerEventCallback;
...
export function initializeHooks() {
  ...
  listener.start();
  setInterval(
    () => overwolf.games.events.getInfo(onUpdate),
    positionUpdateRate
  );
  (window as OverwolfHookWindow).NWMM_registerEventCallback =
    onPlayerDataUpdateEvent.register;
}
