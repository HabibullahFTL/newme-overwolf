...
type OnHotkeyInvokedListener = (
  hotkeyResult: overwolf.settings.hotkeys.OnPressedEvent
) => void;

class HotkeyManager {
  private static _instance: HotkeyManager;
  private hotkeyEvents: Readonly<
    Record<Hotkey, UnloadingEvent<OnHotkeyInvokedListener>>
  >;
  ...
  constructor() {
    ...
    for (const [key, overwolfBindName] of Object.entries(hotkeys)) {
      ...
      OWHotkeys.getHotkeyText(hotkeys.toggleInGame, newWorldId).then(
        (hotkeyText) => {
  ...
  public static get instance(): HotkeyManager {
    if (!this.isSupported) {
      throw new Error(
        "Using HotkeyManager directly in this window is not supported. Use getHotkeyManager instead."
      );
    }
  ...
  public registerHotkey = (
    hotkey: Hotkey,
    listener: OnHotkeyInvokedListener,
...
export function getHotkeyManager() {
  return (overwolf.windows.getMainWindow() as HotkeyManagerWindow)
    .NWMM_HotkeyManager;
}
