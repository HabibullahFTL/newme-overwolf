...
const dynamicSettingsUrl =
  "https://raw.githubusercontent.com/CptWesley/NewWorldMinimap/master/overwolf/dynamicSettings.json";
const isBackground = NWMM_APP_WINDOW === "background";
const actualRetrieval = isBackground
  ? getDynamicSettingsBackground
  : (overwolf.windows.getMainWindow() as DynamicSettingsWindow)
      .NWMM_getDynamicSettingsBackground;
...
export function initializeDynamicSettings() {
  ...
  (window as DynamicSettingsWindow).NWMM_getDynamicSettingsBackground =
    getDynamicSettingsBackground;
  populateSettings();
}
