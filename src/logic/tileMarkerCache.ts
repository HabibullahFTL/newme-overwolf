...
class TileMarkerCache {
  ...
  private onMarkersLoadedEvent = new UnloadingEvent<OnMarkersLoadedListener>(
    "onMarkersLoaded"
  );
  ...
  public static get instance(): TileMarkerCache {
    if (!this.isSupported) {
      throw new Error(
        "Using MarkerCache directly in this window is not supported. Use getMarkerCache instead."
      );
    }
...
export function getTileMarkerCache() {
  return (overwolf.windows.getMainWindow() as MarkerCacheWindow)
    .NWMM_MarkerCache;
}
