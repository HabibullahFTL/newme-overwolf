...
const bringToFrontWindowStates: string[] = [
...
export class BackgroundController {
  ...
  private _gameRunningEvent = new UnloadingEvent<GameRunningEventListener>(
    "gameRunning"
  );
  ...
  public static get instance(): BackgroundController {
    if (!this.isSupported) {
      throw new Error(
        "Using BackgroundController directly in this window is not supported."
      );
    }
  ...
  public async openWindow(window: ConcreteWindow) {
    if (this._openWindows.has(window)) {
      ...
      if (
        ...
        bringToFrontWindowStates.includes(windowState.window_state)
      ) {
        overwolf.windows.obtainDeclaredWindow(window, (wnd) => {
          if (wnd.success) {
            overwolf.windows.bringToFront(wnd.window.id, () => {
              /* Ignore the result of bringToFront */
            });
          } else {
  ...
  public debug_setGameRunning = (running: boolean) => {
    console.log(
      `Attempted to set gameRunning to ${running}, but this was a no-op.`
    );
  };
...
export function getBackgroundController() {
  ...
  return (overwolf.windows.getMainWindow().window as BackgroundControllerWindow)
    .backgroundController;
}

if (BackgroundController.isSupported) {
  BackgroundController.instance.run();
  (window as BackgroundControllerWindow).backgroundController =
